...
with st.container():
    ...
    with c1:
        if st.button("Save Student"):
            if filled:
                path = "D:\\vscoded\\Reps\\Face-Recognition\\TEST_IT\\Attendance-System-Project\\1. Dataset\\"+name+".jpg"
                ...
                df = pd.read_csv("D:\\vscoded\Reps\Face-Recognition\TEST_IT\Attendance-System-Project\\5. Encodings\\encodings.csv")
                en = df["Encodings"].tolist()
                n = df["Persons"].tolist()
                en.append(img_encoding)
                n.append(name)
                df = pd.DataFrame({"Persons":n,"Encodings":en})
                df.to_csv("D:\\vscoded\\Reps\\Face-Recognition\\TEST_IT\\Attendance-System-Project\\5. Encodings\\encodings.csv",index=False)
